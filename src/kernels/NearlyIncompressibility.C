...
#include "NearlyIncompressibility.h"
#include "libmesh/quadrature.h"
#include "MooseMesh.h"
#include "MooseVariable.h"

registerMooseObject("MECHApp", NearlyIncompressibility);
...
NearlyIncompressibility::NearlyIncompressibility(InputParameters const & parameters) :
    Kernel(parameters),
...
NearlyIncompressibility::~NearlyIncompressibility() {
    for (int i=0; i<64; ++i) {
        delete[] _J_lin[i];
        delete[] _Pvol_Lin[i];
    }
    ...
    if (_i==0 && _j==0 && _qp==0)
      {
          ...
          for (int j=0; j< _phi.size() ; ++j)
              for (int qp=0; qp< _qrule->n_points() ; ++qp)
              {
                  ...
                  _Pvol_Lin[j][qp] = _kappa * ( 2.0 * _J[qp] * _J[qp] *_invFtr[qp].contract(H) * _invFtr[qp]
...
Real NearlyIncompressibility::computeQpOffDiagJacobian(unsigned int jvar) {
