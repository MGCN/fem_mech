...
registerMooseObject("MECHApp", MassBalance);

template <>
InputParameters
validParams<MassBalance>()
{
...
MassBalance::MassBalance(InputParameters const &parameters)
    :
...
{
  return (_J[_qp] - 1.0) * _test[_i][_qp] - 0.0 * _grad_u[_qp] * _grad_test[_i][_qp];
}
...
MassBalance::computeQpJacobian()
{
  ...
  return  0.0 * _grad_phi[_j][_qp] * _grad_test[_i][_qp];
}

Real
MassBalance::computeQpOffDiagJacobian(unsigned int jvar)
{
