...
#include "SplitNewmarkStressDivergenceFast.h"
...
registerMooseObject("MECHApp", SplitNewmarkStressDivergenceFast);
...
SplitNewmarkStressDivergenceFast::SplitNewmarkStressDivergenceFast(InputParameters const & parameters) :
...
void
SplitNewmarkStressDivergenceFast::computeOffDiagJacobian(unsigned int jvar)
{
    if (jvar==_disp_x_var)
        computeJacobian();
}
...
void
SplitNewmarkStressDivergenceFast::computeJacobian()
{
        ...
        for (_j=0; _j<_phi.size(); ++_j)
        {
            for (int _qp=0; _qp<_qrule->n_points(); ++_qp)
            {
                ...
                if(_has_pressure)
                    ...
                    _stress_lin[jdim][_j][_qp] =_stress_lin[jdim][_j][_qp];
            }
        }
    }
        ...
        for (int idim=0; idim<_dim; ++idim)
            for (_i=0; _i<_test.size(); ++_i)
                for (_j=0; _j<_test_pres[0].size(); ++_j)
                    for (_qp=0; _qp<_qrule->n_points(); ++_qp)
                        for (int k=0; k<_dim; ++k)
                            _local_BT[idim](_i,_j)+=1./4 *_JxW[_qp] * _coord[_qp]*(_test_pres[0][_j][_qp])*_J[_qp]*_invFtr[_qp](idim,k)*_grad_test[_i][_qp](k);
...
void SplitNewmarkStressDivergenceFast::computeResidual()
{
    for (int d=0; d<_dim; ++d)
    {
    ...
    for (int d=0; d<_dim; ++d)
        for (_i = 0; _i < _test.size(); ++_i)
            for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
            {
                     RealTensorValue const & P =  _P[_qp] +  _P_old[_qp];
                    
                    for (int k=0; k<_dim; ++k){
                        _local_re[d](_i) += _JxW[_qp] * _coord[_qp] * P(d , k) * _grad_test[_i][_qp] (k);

                        // if (_contact)
                        // _local_re[d](_i) += - 1. * _JxW[_qp] * _coord[_qp] * _reaction(k) ;
                     }
    ...
    if(_has_pressure)
    {
        ...
        for (_i =0; _i<_test_pres->size(); ++_i)
            for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
                _local_re_p(_i)+= 1./4 * _JxW[_qp] * _coord[_qp] *(_J[_qp]-1.0)* (_test_pres[0][_i][_qp]);
