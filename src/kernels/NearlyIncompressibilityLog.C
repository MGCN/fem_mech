...
#include "NearlyIncompressibilityLog.h"
#include "libmesh/quadrature.h"
#include "MooseMesh.h"
#include "MooseVariable.h"

registerMooseObject("MECHApp", NearlyIncompressibilityLog);
...
NearlyIncompressibilityLog::NearlyIncompressibilityLog(InputParameters const & parameters) :
    Kernel(parameters),
    ...
    _kappa(getParam<Real>("kappa"))
{
   ...
   for (unsigned int i = 0; i < _numComp; ++i) 
    ...
    if(_numComp >= 2 && _var.number() == (_disp_var[1])) component=1;
    if(_numComp >= 3 && _var.number() == (_disp_var[2]))  component=2;
...
NearlyIncompressibilityLog::~NearlyIncompressibilityLog() {
    for (int i=0; i<64; ++i) {
        delete[] _J_lin[i];
        delete[] _Pvol_Lin[i];
    }
    ...
    if (_i==0 && _j==0 && _qp==0)
      {
          ...
          for (int j=0; j< _phi.size() ; ++j)
              for (int qp=0; qp< _qrule->n_points() ; ++qp)
              {
                  ...
                  _Pvol_Lin[j][qp] = _kappa * (_invFtr[qp].contract(H) * _invFtr[qp] - 1.0 * std::log(_J[qp]) * _invFtr[qp] * H.transpose() * _invFtr[qp]);
              }
        }
...
Real NearlyIncompressibilityLog::computeQpOffDiagJacobian(unsigned int jvar) {
   
    if (jvar==_disp_var[0] || (_numComp >= 2 && jvar==_disp_var[1]) || (_numComp >= 3 && jvar ==_disp_var[2]))
    {
